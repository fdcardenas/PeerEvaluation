    ...
    if(empty($_SESSION['user'])||empty($_SESSION['logged_aux'])||empty($_SESSION['class'])){
        ...
        if($_SESSION['class']!="profesor"){
            ...
            if(empty($_POST['pass'])||empty($_POST['pass2'])||empty($_POST['codigo'])){
                ...
                $query1= @mysql_query('SELECT * FROM profesortbl WHERE usuario="'.mysql_real_escape_string($user).'" AND codigo="'.mysql_real_escape_string($codigo).'"');
                ...
                if ($existe=@mysql_fetch_object($query1)){
                    ...
                    if($pass==$pass2){
                        ...
                        $sql = "UPDATE profesortbl SET contrasenha='$password1' WHERE usuario='$user'";
                        ...
                        echo '<script>window.location="../Front/registro_contrasenha_profesor.php";alert("Las contraseñas ingresadas no son iguales.");</script>'; 
                    }
                }
                else{
