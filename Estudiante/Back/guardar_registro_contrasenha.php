    ...
    if(empty($_SESSION['user'])||empty($_SESSION['logged_aux'])||empty($_SESSION['class'])){
        ...
        if($_SESSION['class']!="estudiante"){
            ...
            if(empty($_POST['pass'])||empty($_POST['pass2'])||empty($_POST['codigo'])){
                ...
                $query1= @mysql_query('SELECT * FROM estudiantetbl WHERE usuario="'.mysql_real_escape_string($user).'" AND codigo="'.mysql_real_escape_string($codigo).'"');
                ...
                if ($existe=@mysql_fetch_object($query1)){
                    ...
                    if($pass==$pass2){
                        ...
                        $sql = "UPDATE estudiantetbl SET contrasenha='$password1' WHERE usuario='$user'";
                        $result = mysql_query($sql) or die("La consulta no se pudo realizar");
                        unset($_SESSION['logged_aux']);
                        echo '<script>window.location="../Front/inicio_estudiante.php";</script>';    

                    }
                    else{
                        echo '<script>window.location="../Front/registro_contrasenha_estudiante.php";alert("Las contraseñas ingresadas no son iguales.");</script>'; 
                    }
                }
                else{
